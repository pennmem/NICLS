...
class DummyTask:
    ...
    async def connect(self):
        ...
        self.writer.write(bytes(TaskMessage("READ_ONLY_STATE", data={"enable": 1})))
        print("Encoding 1")
        self.writer.write(bytes(TaskMessage("ENCODING", data={"enable": 1})))
        await asyncio.sleep(4)
        print("Encoding 2")
        self.writer.write(bytes(TaskMessage("ENCODING", data={"enable": 1})))
        await asyncio.sleep(10)
        print("Read Only State - OFF")
        self.writer.write(bytes(TaskMessage("READ_ONLY_STATE", data={"enable": 0})))
